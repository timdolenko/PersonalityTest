...
class DataTransferServiceTests: XCTestCase {
    ...
    private func makeNetworkService(with data: Data?) -> NetworkService {
        NetworkService(
            config: mockConfig,
        ...
        _ = sut.request(with: mockModelEndpoint, completion: { result in
            ...
            case let .failure(error):
                ...
                guard case DataTransferError.networkFailure(
                    NetworkError.error(statusCode: 500, data: _)) = error else {
                    XCTFail("Wrong error")
                    return
                }
    ...
    func test_whenNoDataReceived_shouldThrowNoDataError() {
        ...
        let networkService = NetworkService(config: mockConfig, sessionManager: sessionManager)
